...
class LoadData:
    def __init__(self, path, evaluation=False):
        if evaluation: #BaoNDQ
        ...
        if self.evaluation:   #BaoNDQ
            gt_df = pd.read_csv(gt_path, header=None)
            gt_df.columns = ['frame', 'id', 'x', 'y', 'width', 'height', 'conf', 'cls', 'vis']
            self.gt = []
            n_frames = gt_df['frame'].unique().shape[0]
            for i in range(n_frames):
                gt_arr = gt_df[gt_df['frame']==i+1].values
                gt_arr = gt_arr[gt_arr[:, -3] == 1]         # conf = 1
                gt_arr = gt_arr[gt_arr[:, -2] == 1]         # cls_id = 1
                gt_arr = gt_arr[gt_arr[:, -1] > 0.5]        # Visibility
                gt_arr = gt_arr[:, 1:6]
    ...
    def __next__(self):
        ...
        # read ground truth for evaluation - BaoNDQ
        if self.evaluation:
            return img, path, self.cap, self.gt[self.count-1]
        else:
            return img, path, self.cap, None
